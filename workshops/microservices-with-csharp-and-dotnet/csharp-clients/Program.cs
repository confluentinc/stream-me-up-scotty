    ...
    class Program
    {
        ...
        static void Produce(string topic, ClientConfig config)
        {
            using (var producer = new ProducerBuilder<string, string>(config).Build())
            {
                ...
                {
                    viewTime = viewTime + 10;
                    var key = viewTime.ToString();
                    var userId = "User_" + rnd.Next(1, 9).ToString();
                    var pageId = "Page_" + rnd.Next(10, 99).ToString();
                    var val = JObject.FromObject(new { viewtime = viewTime, userid = userId, pageid = pageId }).ToString(Formatting.None);
